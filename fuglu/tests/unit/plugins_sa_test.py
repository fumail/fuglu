...
class SATestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        #current tests don't need config options, add them here later if necessary
        self.config = config
    ...
    def test_extract_spamstatus(self):
        ...
        headertests=[ #tuple header content, expected spamstatus, expected spam score
            ...
            ("No, score=1.9 required=8.0 tests=BAYES_00,FROM_EQ_TO,TVD_SPACE_RATIO,TVD_SPACE_RATIO_MINFP autolearn=no autolearn_force=no version=3.4.0", False, 1.9), #with test names, bug #24
        ]

        for headercontent,expectedspamstatus,expectedscore in headertests:
            msgrep=Message()
            msgrep[headername]=Header(headercontent).encode()
            spamstatus,score = candidate._extract_spamstatus(msgrep,headername,suspect)
            self.assertEqual(spamstatus,expectedspamstatus, "spamstatus should be %s from %s"%(expectedspamstatus,headercontent))
            self.assertEqual(score,expectedscore,"spamscore should be %s from %s"%(expectedscore,headercontent))
